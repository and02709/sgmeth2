...
SMDorth_group <- function(X, us, d, u, v, n, p, groups, nonzero.groups, unique.group, 
  ...
  for(iter in 1:niter){
    ...
    # normalize vector u
    u <- matrix(matrix(numer/sgmeth2::l2n(numer)),ncol=1)
    ...
    #   nonzero groups
    lambda <- sgmeth2::calculate.lambda(Xtu.vec=temp$Xtu.vec, 
                               Xk.col.vec=temp$Xk.col.vec, 
                               nonzero.groups=nonzero.groups)
    # Shrink each vector by the approprite amount determined by lambda
    shrunk.list <- sgmeth2::vector.shrinkage(Xtu.list = temp$Xtu.list, Xtu.vec=Xtu.vec, 
