...
gSPCA <- function(X,Y,npc, kernel=c("linear","delta"), groups=NULL, nonzero.groups=NULL, ind.names=NULL, niter=50, trace=F){
  ...
  if(is.null(groups) && is.null(nonzero.groups)){
    return(sgmeth2::SSPCA(X=X,Y=Y,npc=npc,nonzero.loadings = p, sparsity.type="loadings",kernel=kernel, niter=niter, trace=trace))
  }
  
  # Perform SSPCA if no groups are specified but values of nonzero.groups is specified
  if(is.null(groups) && !is.null(nonzero.groups)){
    return(sgmeth2::SSPCA(X=X,Y=Y,npc=npc,nonzero.loadings = nonzero.groups, sparsity.type="loadings", kernel=kernel, niter=niter, trace=trace))
  }
  ...
  # Group decomposition
  out <- sgmeth2::PMD_group(X=Psi,npc=npc,n=n,p=p,groups=groups,
